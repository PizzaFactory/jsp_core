/**
 * \file postmotem592.c
 * \brief ADSP-BF592用のポストモーテムダンプルーチン群
 *
...
void spurious_int_handler()
{
    ...
    unsigned int imask, sic_imask; /*マスク記録レジスタ*/
    ...
    while (1)
    {
        ...
        */
        ...
        pm_putstr( "SIC_IMASK " );  pm_puthex4byte( sic_imask ); pm_putrtn();
        pm_putstr( "SIC_ISR   " );  pm_puthex4byte( *pSIC_ISR ); pm_putrtn();
        ...
        pm_putstr( "TWI_SLAVE_STAT           " );   pm_puthex4byte( *pTWI_SLAVE_STAT ); pm_putrtn();
        pm_putstr( "TWI_MASTER_STAT           " );  pm_puthex4byte( *pTWI_MASTER_STAT ); pm_putrtn();

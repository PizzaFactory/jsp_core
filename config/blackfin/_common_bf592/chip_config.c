...
sys_initialize()
{
     ...
     */
    /*
     *  25MHz*16倍=400MHzに設定する
     *  CSEL = 1; SSEL = 4 (400MHz)
     */

     {

        *pSIC_IWR = 1<<0;               // PLLのみIWRを許す
    // CCLK_DIV1            0x0000
    // CCLK_DIV2            0x0010
    // CCLK_DIV4            0x0020
    // CCLK_DIV8            0x0030
...
#define DLAB 0x80
        ...
        *pUART0_IER = 0;
            ...
            /* ADSP-BF592のUART0をPINに割り当てる　*/
        *pPORTF_MUX &= 0xE7FF;      // bit 11,12を0に
        *pPORTF_FER |= 0x1800;      // bit 11,12を1に


}

/* リセット直後のIVGx SIC_IARx対応表。16エントリがIVG0からIVG15を
 * 表現している。ビットマップはSIC_ISRのビットマップに対応しており、
 * リセット直後、SIC_IARxによってSICのどの割り込みがどの優先順位に
 * 対応しているかを表す。 ADSP-BF59x Blackfin Processor Hardware
 * Referenceの Fig 4-3を参照 */
